...
const Row = ({ data }) => {
  ...
  const select = () => { dispatch({ type: "SELECT", id: data.id }); };
  const remove = () => { dispatch({ type: "REMOVE", item: data }); };
  return (
...
const Main = () => {
  ...
  const runLots = () => { dispatch({ type: "RUN_LOTS" }); };
  const add = () => { dispatch({ type: "ADD" }); };
  const update = () => { dispatch({ type: "UPDATE" }); };
  const clear = () => { dispatch({ type: "CLEAR" }); };
  const swapRows = () => { dispatch({ type: "SWAP_ROWS" }); };
  return (
    <div className="container">
      <div className="jumbotron">
        <div className="row">
          <div className="col-md-6"><h1>React + Redux - No Hooks</h1></div>
